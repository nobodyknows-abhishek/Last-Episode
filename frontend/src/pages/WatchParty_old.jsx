...
const WatchParty = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on("room_state", (state) => {
      ...
      setIsHost(state.host === newSocket.id);
    });
    ...
    newSocket.on("user_joined", (newUser) => {
        // If I am sharing, initiate call to new user
        if (streamRef.current && isHost) {
             callUser(newUser.socketId, streamRef.current, newSocket);
        }
    });
  ...
  // WebRTC Functions
  const createPeer = (targetId, socketInstance) => {
  ...
  const callUser = async (targetId, stream, socketInstance) => {
      if(!socketInstance) return;
      const peer = createPeer(targetId, socketInstance);
  ...
  const handleReceiveOffer = async (data) => {
      if(!socket) return;
      const peer = createPeer(data.sender, socket);
       peersRef.current[data.sender] = peer;
  ...
  const handleReceiveCandidate = async (candidate) => {
      ...
      // The candidate messages usually come with sender ID
      // But here candidate is payload.candidate. Wait, payload structure in server.js:
      // socket.on("ice-candidate", (incoming) => io.to(incoming.target).emit("ice-candidate", incoming.candidate));
      // My server implementation for ice-candidate just emits the candidate object directly?
      // No, let's check server.js edit.
      // socket.on("ice-candidate", (incoming) => io.to(incoming.target).emit("ice-candidate", incoming));
      // Ah, I should pass the whole object including sender.
      // Let's assume incoming has sender.
  };

  // Fix Handle Candidate properly
  // Since I need sender ID to find the peer connection.
  ...
  const startScreenShare = async () => {
      try {
          ...
          // Call all users in room
          roomUsers.forEach(u => {
              if(u.socketId !== socket.id) {
                  callUser(u.socketId, stream, socket);
              }
          });
          ...
          stream.getVideoTracks()[0].onended = () => {
              ...
              peersRef.current = {};
              socket.emit("stop_share", { roomId }); // Optional
          };
      } catch (err) {
